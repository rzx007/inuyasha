...
<template>
  <div class="overflow-y-auto h-full bg-white">
    <!-- 属性配置区域 -->
    <div class="space-y-0">
      <div v-if="propsSchema.length > 0" class="px-4 py-3 bg-slate-50/50 border-b border-slate-100">
        <h4 class="text-xs font-semibold text-slate-500 uppercase tracking-wider">属性配置</h4>
      </div>
      <div
      ...
      >
        <PropField :config="propSchema" @open-binding="handleOpenBinding" />
      </div>
    </div>
    ...
    <div class="space-y-0 border-t border-slate-100">
      <div class="px-4 py-3 bg-slate-50/50 border-b border-slate-100">
        <h4 class="text-xs font-semibold text-slate-500 uppercase tracking-wider">样式配置</h4>
      </div>
      <div
        ...
        class="border-b border-slate-100 last:border-0 px-4 py-4"
      >
        <PropField :config="styleProp" :is-style="true" @open-binding="handleOpenBinding" />
      </div>
    </div>
