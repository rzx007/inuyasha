...
import { useDataSource, useEditor } from '@inuyasha/vue'
...
<template>
  <div class="h-full flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          >
            <div class="flex items-center gap-2 overflow-hidden">
              ...
              <div class="flex flex-col min-w-0">
                <span class="text-xs font-mono font-medium text-slate-700 truncate">{{
      ...
      <div>
        <div class="flex items-center justify-between mb-3">
          <h3
            class="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-2"
          >
            <Globe :size="12" /> API 集成
          </h3>
          <button
            @click="handleAddApi"
            class="p-1 hover:bg-slate-100 rounded text-slate-500 hover:text-indigo-600 transition-colors"
          >
            <Plus :size="14" />
          </button>
        </div>
        ...
        <div class="space-y-2">
          ...
          >
            <div class="flex items-center justify-between mb-2">
              <div class="flex items-center gap-2">
                <span
                  ...
                  :class="getMethodClass((api.config as ApiDataSourceConfig).method)"
                >
            ...
            <div class="flex items-center gap-2 pl-1 mb-1">
              ...
              <span
                class="text-xs font-mono text-slate-500 truncate w-full leading-none"
                :title="(api.config as ApiDataSourceConfig).url"
              >
            ...
            <!-- Status Preview -->
            <div
              v-if="api.lastFetched"
              class="flex items-center gap-2 pl-1 mt-2 pt-2 border-t border-slate-50"
            >
              <span
                class="w-1.5 h-1.5 rounded-full"
                :class="api.data?.error ? 'bg-red-400' : 'bg-emerald-400'"
              ></span>
              <span class="text-[10px] text-slate-400">
                上次更新: {{ new Date(api.lastFetched).toLocaleTimeString() }}
              </span>
              <span v-if="api.data?.error" class="text-[10px] text-red-400 ml-auto"
                >Request Failed</span
              >
      ...
      <div>
        ...
        <div class="space-y-1">
          <div
            ...
            :key="v"
            class="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-slate-100 transition-colors cursor-help group"
          >
            <Database :size="12" class="text-slate-300 group-hover:text-slate-500" />
            <span class="text-xs font-mono text-slate-500 group-hover:text-slate-700">{{
    ...
    <!-- API Form Dialog -->
    <ApiForm v-model:open="showApiForm" :api="currentEditingApi" @save="handleSaveApi" />
        ...
        <div class="flex items-center justify-between border-b border-slate-100 pb-2">
          <DialogTitle class="text-sm font-semibold text-slate-800 flex items-center gap-2">
            <div
              ...
              :class="testResult?.status >= 400 ? 'bg-red-500' : 'bg-emerald-500'"
            ></div>
            API 响应结果
          </DialogTitle>
        </div>
        <div class="flex-1 overflow-auto bg-slate-900 rounded-md p-4 mt-2">
          <pre class="font-mono text-xs text-emerald-400 leading-relaxed whitespace-pre-wrap">{{
